...
import { 
  ...
  Moon, Heart, Utensils, Dumbbell, ChevronDown, ChevronUp, Plus, Trash2, Wallet, CheckCircle2
} from 'lucide-react';
...
export function Diario() {
  ...
  const location = useLocation(); // Para detetar o #hash
  ...
  useEffect(() => {
    // Verifica o hash na URL (ex: #treino) e rola para o elemento
    if (location.hash) {
  ...
  // Estados Financeiro (Novo Módulo Rápido)
  ...
  // ... (Funções de Fetch mantidas iguais) ...
      ...
      if (data) {
        ...
        if (Array.isArray(data.workout_details)) {
            const sanitizedExercises = data.workout_details.map((ex: any) => ({
                ...ex, sets: Array.isArray(ex.sets) ? ex.sets : [{ reps: '', load: '' }, { reps: '', load: '' }, { reps: '', load: '' }]
            }));
            setWorkoutDetails(sanitizedExercises);
        } else setWorkoutDetails([]);
      } else resetForm();
    } catch (error) { console.error(error); } finally { setLoading(false); }
  }

  function resetForm() {
    setWeight(''); setSleep(''); setLibido(''); setDietScore(''); setWorkoutType(''); setWorkoutDetails([]); setSelectedDietId('');
  }
  ...
  const handleSaveDaily = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        weight: weight ? Number(weight) : null, sleep_score: sleep, libido_score: libido, 
        diet_score: dietScore, diet_plan_id: (dietScore === 'off') ? null : (selectedDietId ? Number(selectedDietId) : null),
        workout_type: workoutType, workout_details: workoutType === 'nao_treinei' ? [] : workoutDetails
      };
  ...
  // Nova Função: Salvar Gasto Rápido
  const handleQuickFinance = async () => {
    ...
    try {
      ...
      const { error } = await supabase.from('transactions').insert([{
        ...
        type: 'expense', // Assume despesa por padrão no lançamento rápido
      ...
      alert('Gasto lançado!');
      setFinDesc(''); setFinAmount('');
    } catch (e) { alert('Erro ao lançar gasto'); } finally { setSavingFinance(false); }
  };

  // ... (Helpers de Treino mantidos) ...
  const importWorkout = (workoutId: string) => {
    const ficha = savedWorkouts.find(w => w.id === Number(workoutId));
    if (ficha?.exercises) {
      setWorkoutDetails(ficha.exercises.map(ex => ({ 
        id: crypto.randomUUID(), name: ex.name, notes: ex.notes, 
        sets: Array.from({ length: Number(ex.sets) || 3 }, () => ({ reps: '', load: '', targetReps: ex.reps, targetLoad: ex.load }))
      })));
      setIsWorkoutExpanded(true);
    }
  };
  const updateSet = (exIdx: number, sIdx: number, field: keyof SetData, val: string) => {
    const newD = [...workoutDetails]; newD[exIdx].sets[sIdx][field] = val; setWorkoutDetails(newD);
  };
  ...
  const getHistoryForSet = (name: string, idx: number) => {
    const sets = historyMap[name.trim().toLowerCase()];
    return sets && sets[idx] ? { load: sets[idx].load, reps: sets[idx].reps } : null;
  };
        ...
        loading ? <div style={{ textAlign: 'center', padding: '4rem' }}><Loader2 className="animate-spin" size={40} /></div> : (
          <div style={{ display: 'grid', gap: '1rem' }}>
            ...
            <form onSubmit={handleSaveDaily}>
              <Card>
                {/* 1. MÉTRICAS (Ref para scroll) */}
                <div id="metrics" ref={metricsRef} className={styles.sectionContainer}>
                  <div className={styles.sectionTitle}>Métricas Básicas</div>
                  <div className={styles.inputGroup}><Input label="Peso (kg)" type="number" step="0.05" value={weight} onChange={e => setWeight(e.target.value)} /></div>
                ...
                {/* 2. FINANCEIRO RÁPIDO (Ref para scroll) */}
                <div id="financeiro" ref={financeRef} className={`${styles.sectionContainer} ${styles.financeSection}`}>
                ...
                {/* 3. NUTRIÇÃO (Ref para scroll) */}
                <div id="nutricao" ref={nutritionRef} className={`${styles.sectionContainer} ${styles.nutritionSection}`}>
                ...
                {/* 4. TREINO (Ref para scroll) */}
                <div id="treino" ref={workoutRef} className={`${styles.sectionContainer} ${styles.workoutSection}`}>
                  <QuickSelector label="Treino" icon={<Dumbbell size={18} />} value={workoutType} onChange={(v: string) => { setWorkoutType(v); setIsWorkoutExpanded(true); }} options={[{ value: 'nao_treinei', label: 'Off', color: '#94a3b8' }, { value: 'alternativo', label: 'Outro', color: '#ec4899' }, { value: 'academia', label: 'Gym', color: '#3b82f6' }]} />
                ...
                <div className={styles.footerActions}>
                  <Button type="submit" disabled={saving}>{saving ? <Loader2 className="animate-spin" /> : <><Save size={18} /> Salvar Tudo</>}</Button>
                </div>
              </Card>
            </form>
          </div>
